...
class Utils {
    ...
    /**
     * Permite devolver true o false de forma aleatoria. Esto se hizo en
     * principio para saber cual bloque de anuncios mostrar.
     * @return boolean
     */
    ...
    /**
     * Extrae la ruta de la primer imagen dado un texto en HTML
     */
    ...
    /**
     * Extrae el nombre de la primer imagen dado un texto en HTML
     */
    public static function getThumbFromImage($html) {
        ...
        foreach ($imgs as $img) {
            Logger::debug("Utils.getThumbFromImage(...) img->getAttribute('src') = {$img->getAttribute('src')}");
            Logger::debug("Utils.getThumbFromImage(...) path = $path");
            $pos = stripos($img->getAttribute('src'), $path);
            Logger::debug("Utils.getThumbFromImage(...) pos = $pos");
            if ($pos !== FALSE) {
                return str_replace($path, '/upload/.thumbs/image/', $img->getAttribute('src'));
            }
        }
        return FALSE;
    }
    ...
    {
        Load::models('blog/blog_etiqueta');
        $etiqueta = new BlogEtiqueta();
        $tags = array();
        if($id_post){
